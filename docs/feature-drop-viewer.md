...
- Loaded models persist per browser tab so reloading a specific editor restores the complete list of meshes that tab imported (not just the most recent one). New tabs still seed from the most recent successful import—mirroring the entire list—then diverge once they start adding their own assets. The persistence layer stores base64 data URLs plus file names and is cleared if restoration fails.
