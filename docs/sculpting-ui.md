...
- With the Smooth tool active the viewport previews the affected area using the GPU light-mask defined in `docs/sculpt-highlight.md`: a soft additive glow centered on the hit point with a fading edge. The overlay follows the same screen-space radius logic, updates continuously as the mesh deforms, and disappears as soon as the sculpt tool is deselected or the pointer leaves the surface.
- Dragging with the Smooth tool continuously averages the vertices under the active pointer, so the surface is gently blended in real time without any extra confirmation steps.
...
- Gestures: primary/left drag sculpts; Alt/Option + left drag or middle/right drag orbits/pans; wheel/trackpad scroll zooms. Double click starts camera movement on desktop. Touch stays single-finger sculpt-only; multi-finger gestures are ignored.
