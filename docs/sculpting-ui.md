...
- With the Smooth tool active the viewport now previews the affected area by tinting all triangles within the current brush radius beneath the pointer. The overlay follows the same screen-space radius logic, updates continuously as the mesh deforms, and disappears as soon as the sculpt tool is deselected or the pointer leaves the surface.
